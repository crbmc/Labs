
public class AESCipher {

public static String[] aesRoundKeys(String KeyHex) {
	String K[][] = new String[4][4];
	String W[][] = new String [4][44];
	String [] results = new String[11];
	
	//step 1
	String [] temp = new String[16];
	for(int i = 0; i<KeyHex.length(); i= i+2) {
		temp[i/2] = (KeyHex.substring(i,i+2));
	}
	
	for(int i = 0, j = 0; i<temp.length; i = i+4, j++) {
		K[0][j] = temp[i];
		K[1][j] = temp[i+1];
		K[2][j] = temp[i+2];
		K[3][j] = temp[i+3];
	}
	
	//step 2
	for(int i = 0; i < 4; i++) {
		W[0][i] = K[0][i];
		W[1][i] = K[1][i];
		W[2][i] = K[2][i];
		W[3][i] = K[3][i];
		
	}
	// Part 3 (a) 
	for(int j = 4; j<44; j++) {
		
		// if not multiple of 4
		if(j % 4 != 0) {
			for(int i = 0; i < 4; i++) {
				int w4 = Integer.parseInt(W[i][j-4], 16);
				int w5 = Integer.parseInt(W[i][j-1], 16);
					
				W[i][j] = Integer.toHexString(w4 ^ w5);
				}
			// if multiple of 4; part (b) call aesRcon method
		}else {
			String[] Wnew = new String[4];
			for(int i = 0; i<4; i++)
				Wnew[i] = W[i][j-1]; // Wnew[i] is element in array
				
				// shifting rows
				String a = Wnew[0];
				
			for(int i = 0; i<3; i++) {
				Wnew[i] = Wnew[i+1];
				}
				Wnew[3] = a;
			
				// transform Wnew  with Sbox function
			for(int i = 0; i<4; i++) {
				Wnew[i] = aesSBox(Wnew[i]);
				}
				
				int w6 = Integer.parseInt(aesRcon(j/4), 16);
				int w7 = Integer.parseInt(Wnew[0], 16);
				
				Wnew[0] = Integer.toHexString(w6 ^ w7);
				
				
				// final transformation
			for(int i = 0; i < 4; i++) {
				int w4 = Integer.parseInt(W[i][j-4], 16);
				int w5 = Integer.parseInt(Wnew[i], 16);	
				W[i][j] = Integer.toHexString(w4 ^ w5);
				}	
		}
	}

	//Separate ind round keys
		
//	for(int i = 0;  i< W.length; i += 4) {
//		StringBuffer str = new StringBuffer();
//		for(int j = 0; j<43; j++) {
//			str.append(W[0][j]);
//			str.append(W[1][j]);
//			str.append(W[2][j]);
//			str.append(W[3][j]);
//		}
//		results[] = str;
		
	StringBuffer str = new StringBuffer();
	for(int j = 0; j<43; j++) {
		if (j%4==0) {
			results[j/4] = str.toString().toUpperCase();
			str.delete(0, str.length());
		}
		str.append(W[0][j]);
		str.append(W[1][j]);
		str.append(W[2][j]);
		str.append(W[3][j]);
		
	}
	//for (String s: results)
		//System.out.println(s);
	
    // final step
	
	return results;
}
public static String aesSBox(String inHex) {
	int it = Integer.parseInt(inHex, 16);
	char x = s[it];

	 return Integer.toHexString((int) x);
	
}
/*public static String[][] aesSBox(String inHex) {
 * 
	String[][] K = new String[4][4];
	String [] temp = new String[16];
	for(int i = 0; i<inHex.length(); i= i+2) {
		temp[i/2] = (inHex.substring(i,i+2));
	}
	for(int i = 0, j = 0; i<temp.length; i = i+4, j++) {
		K[j][0] = temp[i];
		K[j][1] = temp[i+1];
		K[j][2] = temp[i+2];
		K[j][3] = temp[i+3];
	}
	return K;
}

static String sBoxMe(String in) {
	
	int it = Integer.parseInt(in, 16);
	char x = s[it];

	 return Integer.toHexString((int) x);
}*/

static String aesRcon(int round) {
	String hex = String.format("%04x",  (int) rcon[round]).substring(2,4);
	return hex;
}

public static String[][] AESStateXOR(String [][] sHex, String [][] keyHex) {
	
	String [][] outStateHex = new String[4][4];
	for(int i = 0; i < 4; i++) {
		for(int j = 0; j < 4; j++) {
		  int w4 = Integer.parseInt(sHex[i][j], 16);
		  int w5 = Integer.parseInt(keyHex[i][j], 16);
		
		outStateHex[i][j] = Integer.toHexString(w4 ^ w5).toUpperCase();
		}
	}
	return outStateHex;
	
}

public static String[][] AESNibbleSub(String [][] inStateHex){

	  String [][] outStateHex = new String[4][4];
	  	for(int i = 0; i<4; i++) {
	  		for(int j = 0; j<4; j++) {
	  			outStateHex[i][j] = aesSBox(inStateHex[i][j]).toUpperCase();
	  		}
	  	}
	  	return outStateHex;
}

public static String [][] AESShiftRow(String [][] inStateHex){
	String [][] outStateHex = new String [4][4];
	// row 1
		for(int i = 0; i<4; i++) {
				outStateHex[0][i] = inStateHex[0][i].toUpperCase();
		}
		// row 2
		for(int i = 0; i<3; i++) {
			outStateHex[1][i] = inStateHex[1][i+1];
		}
		outStateHex[1][3] = inStateHex[1][0];
		
		// row 3
		for(int i = 0; i<2; i++) {
			outStateHex[2][i] = inStateHex[2][i+2];
		}
		outStateHex[2][2] = inStateHex[2][0];
		outStateHex[2][3] = inStateHex[2][1];
		
		// row 4
		outStateHex[3][0] = inStateHex[3][3];
		outStateHex[3][1] = inStateHex[3][0];
		outStateHex[3][2] = inStateHex[3][1];
		outStateHex[3][3] = inStateHex[3][2];
		
	return outStateHex;
}

/*public static String [][] AESMixColumn(String [][] inStateHex){
	String [][] outStateHex = new String [4][4];
	
	for(int i = 0; i<4; i++)
		for(int j = 0; j<4; j++)
			outStateHex[i][j] = inStateHex[i][j] ^ gf2[j][j];*/
}


private static final char[] rcon= {
		
	    
};

private static final char [] s = {
		 
         };

/*private static final String [][] gf2 = {
		
};*/



